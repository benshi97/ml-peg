"""Run calculations for CMRAds200 benchmark."""
...
def test_cmrads200(mlip: tuple[str, Any]) -> None:
    """
    Benchmark the CMRAds200 dataset.

    Adsorption energy calculations for 200 systems involving 8 different
    adsorbates on 25 different transition metal surfaces at full coverage.
    Reference values are taken from
    https://cmr.fysik.dtu.dk/adsorption/adsorption.html#adsorption.
    Cite: J. Phys. Chem. C 2018, 122, 8, 4381-4390.

    - Each system consists of surface, molecule+surface, adsorbate molecule,
      and in some cases a product molecule:
    ...
    - Computes adsorption energy =
      E(sum of products on the right) - E(reactants on the left)
    ...
    for idx, reaction_id in enumerate(
        tqdm(cmrads_reactions_dict.keys(), desc="Evaluating model on triplets")
    ):
